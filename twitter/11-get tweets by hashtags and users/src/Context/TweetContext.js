...
export {TweetProvider, useTweetState, useTweetDispatch,UpdateHastgLists,setLikeTweet,setHashtagList ,setTweetText,setTweetList};
...
function setHashtagList(dispatch,data) {
  dispatch({
    type: "SET_HASHTAG_LISTS",
    payload:data
  });
}
function UpdateHastgLists(dispatch) {
  getAllHashtags((isok,data)=>{
    if(!isok)
    return alert("مشکلی پیش آمده")
    dispatch({
      type: "SET_HASHTAG_LISTS",
      payload:data
    });
