...
import "../PointsSlider/points-slider.less";
import {
  getCustomerSliderStartDate,
  getCustomerSliderEndDate,
  setCustomerLoadMore,
} from "../../containers/customerList/action";
...
export default function CustomerListSlider({ firstTransaction, setLoading }) {
  ...
  useEffect(() => {
    dispatch(getCustomerSliderStartDate(convertToDate(start_date)));
    dispatch(getCustomerSliderEndDate(convertToDate(end_date)));
  }, []);
  ...
  const handleChange = (event, newValue) => {
    setValue(newValue);
    dispatch(setCustomerLoadMore(false));
  };
