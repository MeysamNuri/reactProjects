...
const ShowTodo = (props) => {
    ...
    const editHandlerTodo=(text)=>{
        axios.put(`https://react-course2.firebaseio.com/todos/${item.key}.json`,{done:item.done,text})
        .then(response=>{
    ...
    let doneHandler=(e)=>{
        axios.put(`https://react-course2.firebaseio.com/todos/${item.key}.json`,{done:!item.done,text:item.text})
        .then(response=>{
